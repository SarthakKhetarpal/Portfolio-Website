...
export const ProjectModalCard = ({ projectModal, currentMode, skillsImg, projectVideo }) => {
  ...
  return (
    <div className={`flex flex-col lg:flex-row w-[100%] h-[90%] gap-y-0 space-y-0 lg:h-fit overflow-y-scroll mx-auto my-auto`}>
      ...
      <div className={`lg:w-[30%] w-[100%] ${currentMode === "light" ? "bg-white" : "bg-richblack-1000"} h-full lg:h-auto p-4 flex flex-col gap-5 overflow-y-scroll`}>
        ...
        <p className={`text-sm italic px-1 ${currentMode === "dark" ? "text-white" : "text-richblack-500"}`}>
          {projectModal.desc}
        </p>
        <p className='font-medium text-sm'>
          Technical Skills Used :
        </p>
        <div className='flex flex-row flex-wrap gap-x-4 gap-y-6 py-2 px-1'>
          {skillsImg.map((projectSkill, index) => {
            // Extract the URL from the object
            const skillUrl = Object.values(projectSkill)[0];
            return (
              <div key={index} className='cursor-pointer group'>
                <img src={skillUrl} width={45} alt='skill' className='group-hover:scale-125' />
              </div>
            )
          })}
        </div>
        ...
        <a href={projectModal.url} className='cursor-pointer group'>
          <img src={github.current} alt='GitHub-Icon-URL' width={50} className='group-hover:scale-110' />
        </a>
      </div>
    </div>
  );
